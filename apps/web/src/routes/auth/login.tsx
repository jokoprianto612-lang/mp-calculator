...
function LoginPage() {
  ...
  const onSubmit = async (data: LoginForm) => {
    try {
      ...
      toast.success(t('notifications.sessionExpired')); // Using existing key
      navigate({ to: '/calculator' });
    } catch (error) {
      toast.error(error instanceof Error ? error.message : t('auth.validation.emailInvalid'));
    }
  };
      ...
      <div className="w-full max-w-md">
        ...
        <div className="text-center mb-8">
          ...
          <p className="text-slate-600 dark:text-slate-400">
            {t('auth.hasAccount')} <Link to="/register" className="text-primary-600 hover:underline font-medium">{t('auth.signUp')}</Link>
          </p>
        </div>
